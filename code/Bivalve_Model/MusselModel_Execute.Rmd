...
# interp1 is struggling because we have to give it vectors of the same length (18) ##
...
# mld_daily <- matrix(nrow = length(mld_monthly[[1]]), ncol = ncol(mld_monthly))
# 
# # Convert each column to a numeric vector and apply interp1(), and fill the matrix with the interpolated values
# for (i in 1:nrow(mld_monthly)) {
#   row_vector <- as.numeric(mld_monthly[[, i]])
#   interp_vector <- interp1(1:18, row_vector, 1:0.0311:18, method = "pchip")
#   mld_daily[, i] <- interp_vector
# }

  
  Temp_monthly <- celldata[
    # cellno, 
    22:39] + 273
