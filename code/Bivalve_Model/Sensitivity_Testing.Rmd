...
seg_long <- segmented(long_lm, seg.Z = ~ y, npsi = 1, control = seg.control(display = FALSE))

#################################################

xp_long <- c(min(colo_df$y), seg_long$psi[, "Est."], max(colo_df$y))
yp_long <- predict(seg_long, newdata = data.frame(x = xp_long), se.fit = TRUE) # this doesn't work????
...
xp_lat <- c(min(colo_df$x), seg_lat$psi[, "Est."], max(colo_df$x))
```
