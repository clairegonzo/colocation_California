...
plot.segmented(seg_long, res=TRUE, col=2, conf.level=.95)
## Try to clean this up and add new axis titles ##
...
plot.segmented(seg_lat, res=TRUE, col=2, conf.level=.95)
```



Holding for later in case the break point analysis doesn't work: Predict values of the lms so that we can plot them as a line
```{r, eval=FALSE}
long.predict <- cbind(colo_df, predict(long_lm, interval = 'confidence'))

ggplot(data = long.predict, aes(x = x, y = Band_1)) +
  geom_point() +
  geom_line(aes(x = x, y = fit), color = "gray") +
