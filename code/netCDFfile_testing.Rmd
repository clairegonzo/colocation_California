...
for(i in 1:length(mix_files)){
  ...
  for(j in 1:length(matrix)){
    ...
    extract_metric[j] <- extract(r, SpatialPoints(cbind(temp_lon, temp_lat)), method='simple')
    
    ## trying to create a vector with all the extracted metrics from all the coordinates 1:length(matrix) ##
    ## this vector is one month of data for all 5,000 site locations ##
    # vec <- ## Blah i dont know how to do it
  }
  ## trying to basically cbind all the new vectors together, month after month
 # holding$i <- vec ## also stuck here
