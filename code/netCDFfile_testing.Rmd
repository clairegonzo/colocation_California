...
curvel_merged <- base::merge(zonal_curvel_bind, meridional_curvel_bind, by = c("sites_lon", "sites_lat", "months")) %>% 
  pivot_longer(cols = starts_with("vel_"), names_to = "velocity_type", values_to = "velocity") %>% 
  # maybe we don't use group_by at all? Maybe just mutate? # And maybe dont need pivot_longer for that?
  group_by(sites_lon, sites_lat, months, velocity_type) %>% 
  summarize()
...
data_curvelpoc <- data.frame(
  base::merge(data_curvel, matrix_poc, by = c("sites_lon", "sites_lat"))
)
